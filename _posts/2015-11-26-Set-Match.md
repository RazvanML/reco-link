...
with the helf of ```GROUP_CONCAT``` function.

Data import is symmetric and similar to the previous examples, with the exeption of the ```fkeys``` tag, which models the relation between entities:
...
The linkage rule introduce the ```RELATION_SIMILARITY``` rule type. The
...
are matched instead. The two relations shall have the same cardinality.
